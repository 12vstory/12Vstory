  ...
  if(detail){
    ...
    detail.innerHTML = `
      ...
      <div class="content-grid">
        <article class="content-panel"><h2>작업 내용</h2><p>${esc(work.description || work.summary || '')}</p>${tags?`<div class="chips detail-tags">${tags}</div>`:''}</article>
        <aside class="content-panel"><h2>상담 안내</h2><p>같은 차종이라도 연식과 옵션에 따라 작업 방식이 달라질 수 있습니다. 차종·연식·증상을 함께 보내주세요.</p><div class="actions" style="justify-content:flex-start"><a class="btn talk" href="http://talk.naver.com/WC6P8L" target="_blank" rel="noopener">네이버 톡톡</a><a class="btn dark" href="tel:031-912-8812">전화 문의</a></div></aside>
      </div>
      ${imgs?`<section class="detail-gallery"><div class="section-head"><div><div class="kicker">Work Photos</div><h2>작업사진</h2></div></div><div class="photo-grid">${imgs}</div></section>`:`<div class="photo-placeholder">작업사진을 추가하면 이곳에 자동으로 표시됩니다.</div>`}
