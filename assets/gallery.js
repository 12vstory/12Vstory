(function(){
  ...
  function card(work){
    ...
    return `<a class="work-card gallery-item" data-category="${esc(work.category)}" href="work.html?id=${encodeURIComponent(work.id)}">${visualHtml(work)}<div class="body"><small>${esc(work.category)} · ${esc(fmtDate(work.date))}${source}</small><h3>${esc(cardTitle(work))}</h3><p>${esc(cleanSummary(work))}</p></div></a>`;
  }
  ...
  if(detail){
    ...
    detail.innerHTML = `
      <header class="page-hero work-detail-hero"><div class="kicker">${esc(work.category)} · ${esc(fmtDate(work.date))}</div><h1>${esc(displayCar(work))}<br>${esc(workName(work))}</h1><p>${esc(cleanSummary(work))}</p></header>
